...
def main(page: ft.Page):
    ...
    def open_settings(e):
        name_field = ft.TextField(
            label="Name", value=settings.get("user_name", ""),
            bgcolor="#18181C", color=ft.Colors.WHITE, border_color="#3A3A46",
        )
        phone_field = ft.TextField(
            label="Phone Number", value=settings.get("user_phone", ""),
            keyboard_type=ft.KeyboardType.PHONE,
            bgcolor="#18181C", color=ft.Colors.WHITE, border_color="#3A3A46",
        )
        email_field = ft.TextField(
            label="Email", value=settings.get("user_email", ""),
        ...
        threads_field = ft.TextField(
            label="Threads", value=str(settings.get("threads", 4)),
            keyboard_type=ft.KeyboardType.NUMBER,
            bgcolor="#18181C", color=ft.Colors.WHITE, border_color="#3A3A46",
        )
        ...
        try:
            ...
            mem_facts = ft.Text("", size=12); mem_msgs = ft.Text("", size=12)
        ...
        try:
            ...
            if shown:
                ...
                emo_lines = [ft.Text(f"{k}: {v}", size=12, color=ft.Colors.PURPLE_200) for k, v in sorted_emo]
            else:
        ...
        try:
            ...
            if beliefs:
                identity_lines = [
                    ft.Text(f"• [{b['category']}] {b['statement']} "
        ...
        try:
            ...
            if social:
                ...
                if social.get("notes"):
                    social_lines.append(ft.Text(f"Notes: {social['notes']}", size=11, color=ft.Colors.GREY_400))
            else:
        ...
        try:
            ...
            if refl_recent:
                ...
                for ts, kind, summary in refl_recent:
                    reflection_lines.append(ft.Text(f"[{kind}] {summary}", size=11, color=ft.Colors.TEAL_100))
            else:
        ...
        try:
            ...
            if drives:
                ...
                motivation_lines = [ft.Text(f"{k}: {v}", size=12, color=ft.Colors.ORANGE_200) for k, v in sorted_drives]
            else:
        ...
        try:
            ...
            if trace:
                cognition_lines = [
                    ft.Text(f"Focus: {', '.join(k for k in trace.get('focused_on', {}))}",
                            size=11, color=ft.Colors.BLUE_200),
                    ft.Text(f"Appraisal: threat {round(trace.get('appraisal', {}).get('threat', 0), 2)}, "
                            ...
                            size=11, color=ft.Colors.BLUE_200),
                    ft.Text(f"Anticipation: {trace.get('prediction', {}).get('ready_for', '—')}",
                            size=11, color=ft.Colors.BLUE_200),
                    ft.Text(f"Decision: intent={trace.get('decision', {}).get('intent', '—')}, "
        ...
        try:
            ...
            if err_summary:
                for direction, count in err_summary.items():
                    learning_lines.append(ft.Text(f"Errors — {direction}: {count}", size=11, color=ft.Colors.LIME_200))
            else:
            ...
            if prefs:
                ...
                for topic, feeling, seen in prefs[:8]:
                    learning_lines.append(ft.Text(f"• {topic}: {round(feeling, 2)} (seen {seen})",
            ...
            if best:
                ...
                for intent, tone, score, used in best:
                    learning_lines.append(ft.Text(f"Best: {intent}/{tone} — score {round(score, 2)} (used {used})",
                                                  size=11, color=ft.Colors.LIME_200))
        ...
        try:
            ...
            if traits:
                personality_lines = [ft.Text(f"{k}: {v}", size=12, color=ft.Colors.PINK_200)
        ...
        try:
            ...
            if values:
                evolution_lines.append(ft.Text("— Values —", size=11, color=ft.Colors.CYAN_200))
                for k, v in sorted(values.items(), key=lambda x: -x[1]):
                    evolution_lines.append(ft.Text(f"{k}: {v}", size=11, color=ft.Colors.CYAN_200))

            vhistory = response_engine.value_history(limit=5)
            if vhistory:
                evolution_lines.append(ft.Divider(height=1))
                evolution_lines.append(ft.Text("— Recent shifts —", size=11, color=ft.Colors.CYAN_100))
                for value_name, delta, reason, ts in vhistory:
                    sign = "+" if delta >= 0 else ""
                    evolution_lines.append(ft.Text(f"{value_name}: {sign}{round(delta, 4)} ({reason})",
        ...
        try:
            ...
            integration_lines.append(ft.Text(f"Local backups: {backup_count}", size=12, color=ft.Colors.CYAN_200))
            ...
            integration_lines.append(ft.ElevatedButton("Backup Database",
                                                        icon=ft.Icons.SAVE,
                                                        on_click=do_backup,
                                                        width=340))
            integration_lines.append(ft.Text("Instagram: Not connected", size=12, color=ft.Colors.GREY_400))
        ...
        async def pick_model(ev):
            ...
            await file_picker.pick_files(allow_multiple=False, file_type=ft.FilePickerFileType.ANY)
