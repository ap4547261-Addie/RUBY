...
class Curiosity:
    ...
    def _notice_people(self, text, lower):
        relationship_words = (
            "friend", "brother", "sister", "mother", "mom",
            "father", "dad", "cousin", "uncle", "aunt",
    ...
    def _notice_events(self, text, lower):
        event_markers = (
            "yesterday", "today", "last night", "last week",
    ...
    def _notice_experience(self, text, lower):
        experience_markers = (
            "when i was", "when i used to", "growing up",
    ...
    def _add_interest(self, category, subject, reason, strength=0.5):
    ...
    def _add_unknown(self, category, subject, reason, strength=0.5):
        strength = self._clamp(strength)
    ...
    def _resolve_unknown(self, category, subject):
        remaining = []
        for item in self._unknowns:
            same_category = item["category"] == category
            same_subject = item["subject"].lower() == subject.lower()
    ...
    def mark_understood(self, subject):
        ...
        self._uncertainties = [
            item for item in self._uncertainties
    ...
    def mark_explored(self, subject, category=None):
        if not subject:
            return
    ...
    def increase(self, subject, amount=0.15, reason="", category="general"):
        if not subject:
            return

        for item in self._interests:
            if item["subject"].lower() == str(subject).lower():
                item["strength"] = self._clamp(item["strength"] + amount)
                return
